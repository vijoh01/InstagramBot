...
app.post('/follow', async () => {
    const hashtag = 'fitness';
  const searchResults = await ig.feed.tags.search(hashtag);

  if (searchResults && searchResults.length > 0) {
    const recentMedia = await ig.feed.tags.recent(searchResults[0]);

    for (const media of recentMedia) {
      await ig.friendship.create(media.account.id);
      console.log(`Followed user: ${media.account.username}`);
    }
  }
})
